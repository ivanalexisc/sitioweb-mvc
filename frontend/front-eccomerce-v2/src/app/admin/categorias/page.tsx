...
export default function AdminCategorias() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                await http.put(`/categories/${editingId}`, { genero });
            } else {
                await http.post("/categories", { genero });
            }
            resetForm();
            fetchCategories();
        } catch (err: any) {
                ...
                <div className="bg-white rounded-xl border border-gray-100 p-6">
                    ...
                    <form onSubmit={handleSubmit} className="flex items-end gap-3">
                        <div className="flex-1">
                            <label className="block text-sm font-medium text-gray-600 mb-1">Género / Nombre</label>
                            <input
                                type="text"
                                value={genero}
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {categories.map((cat) => (
                                <tr key={cat.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{cat.genero}</td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button onClick={() => openEdit(cat)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors cursor-pointer">
                                                <HiOutlinePencil size={16} />
                                            </button>
                                            <button onClick={() => handleDelete(cat.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer">
