...
export default function AdminColores() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                await http.put(`/colors/${editingId}`, { nombre });
            } else {
                await http.post("/colors", { nombre });
            }
            resetForm();
            fetchColors();
        } catch (err: any) {
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">{colors.length} colores en total</p>
                </div>
            ...
            {showForm && (
                <div className="bg-white rounded-xl border border-gray-100 p-6">
                    <h3 className="text-sm font-semibold text-gray-700 mb-3">
                        {editingId ? "Editar color" : "Crear color"}
                    </h3>
                    <form onSubmit={handleSubmit} className="flex items-end gap-3">
                        <div className="flex-1">
                            <label className="block text-sm font-medium text-gray-600 mb-1">Nombre del color</label>
                            <input
                                type="text"
                                value={nombre}
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {colors.map((color) => (
                                <tr key={color.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4 text-sm text-gray-400 font-mono">{color.id}</td>
                                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{color.nombre}</td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button onClick={() => openEdit(color)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors cursor-pointer">
                                                <HiOutlinePencil size={16} />
                                            </button>
                                            <button onClick={() => handleDelete(color.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer">
                            ...
                            {colors.length === 0 && (
                                <tr>
                                    <td colSpan={3} className="px-6 py-12 text-center text-sm text-gray-400">
                                        No hay colores. Creá el primero.
                                    </td>
                                </tr>
                            )}
                        </tbody>
